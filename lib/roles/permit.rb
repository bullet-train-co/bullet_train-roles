...
module Roles
  module Permit
    def permit(user, through:, parent:, debug: false, intermediary: nil, rails_cache_key: nil, enable_db_cache: false)
      ...
      permissions = if rails_cache_key
        Rails.cache.fetch(rails_cache_key) do
          build_permissions(user, through, parent, intermediary, enable_db_cache)
        end
      else
        build_permissions(user, through, parent, intermediary, enable_db_cache)
      end
      ...
      begin
        ...
        if rails_cache_key
          ...
          permissions = build_permissions(user, through, parent, intermediary, enable_db_cache)
          assign_permissions(permissions)
        else
          raise e
        end
    ...
    def build_permissions(user, through, parent, intermediary, enable_db_cache)
      added_roles = Set.new
      permissions = []
      user.send(through).map(&:roles).flatten.uniq.each do |role|
        unless added_roles.include?(role)
          ...
          permissions += add_abilities_for(role, user, through, parent, intermediary, enable_db_cache)
          added_roles << role
        end
        ...
        role.included_roles.each do |included_role|
          unless added_roles.include?(included_role)
            ...
            permissions += add_abilities_for(included_role, user, through, parent, intermediary, enable_db_cache)
          end
        end
      end
    ...
    def add_abilities_for(role, user, through, parent, intermediary, enable_db_cache)
      permissions = []
      role.ability_generator(user, through, parent, intermediary, enable_db_cache) do |ag|
