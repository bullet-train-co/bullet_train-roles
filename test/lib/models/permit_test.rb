...
class PermitTest < ActiveSupport::TestCase
  class ClassMethodsTest < ActiveSupport::TestCase
    ...
    test "Permit#build_permissions returns an array of Hashes" do
      assert @admin_ability.build_permissions(@admin_user, :memberships, :team, nil, false).is_a?(Array)
      assert @admin_ability.build_permissions(@admin_user, :memberships, :team, nil, false).first.is_a?(Hash)
    end
